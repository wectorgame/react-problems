...
const generateData = (count: number): ListItem[] => {
  return Array.from({ length: count }, (_, i) => ({
    ...
    description: `Описание пользователя ${i + 1}. Это очень длинное описание, которое может занимать несколько строк и содержать много текста для демонстрации производительности.`
  }))
}

const ListItem = ({ item, index }: { item: ListItem; index: number }) => {
  ...
  return (
    <div style={{
      ...
      <div>
        ...
        <p style={{ margin: '0', fontSize: '14px', color: '#888' }}>{item.description}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        <p>
          Список из 10,000 элементов рендерится полностью, что вызывает серьезные проблемы производительности.
          При скролле все элементы перерендериваются, что приводит к лагам.
        </p>
      </div>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <ul>
          ...
          <li>Поиск работает медленно из-за большого количества DOM элементов</li>
